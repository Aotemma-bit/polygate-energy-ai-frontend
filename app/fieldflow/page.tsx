...
export default function FieldFlowPage() {
  ...
  async function askFieldFlow() {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/fieldflow/ask/stream`,
        {
          method: "POST",
          headers: {
      ...
      if (!res.ok || !res.body) {
        throw new Error("FieldFlow streaming request failed.");
      }
        ...
        for (const event of events) {
          ...
          if (eventType === "sources" && data) {
            try {
              ...
              console.error("Unable to parse FieldFlow sources.");
            }
          }
      ...
      setResponse({
        ...
        error:
          error instanceof Error
            ? error.message
            : "Unable to connect to FieldFlow backend.",
      });
    } finally {
      setLoading(false);
    }
